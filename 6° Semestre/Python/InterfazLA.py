...
class OperacionesConCadenas:
    ...
    def GenerarLenguaje(self, lenguaje,Total):
        secuencias = []
        for _ in range(Total):
            secuencia = ''.join(random.choice(lenguaje) for _ in range(random.randint(1, 5)))
            secuencias.append(secuencia)

        return secuencias  # Retorna la lista de secuencias en lugar de contenacion
...
class InterfazOperaciones(tk.Tk):
    def __init__(self):
        ...
        self.ejercicio5 = ttk.Frame(self.ventana)
        ...
        self.ventana.add(self.ejercicio5, text="Ejercicio 5")
    ...
    def Pestaña5(self):
        lblLenguaje = ttk.Label(self.ejercicio5, text="Ejercicio 5")
        lblLenguaje.grid(row=1, column=0)

        lblVocabulario = ttk.Label(self.ejercicio5, text="Lenguaje (separe los elementos por comas):")
        lblVocabulario.grid(row=2, column=1)
        self.txtLenguaje = ttk.Entry(self.ejercicio5)
        self.txtLenguaje.grid(row=3, column=1)
        self.txtLenguaje.bind("<KeyRelease>", self.verificar_contenido5)
        lblTotal = ttk.Label(self.ejercicio5, text="Total")
        lblTotal.grid(row=4, column=1)
        self.txtTotal = ttk.Entry(self.ejercicio5)
        self.txtTotal.grid(row=5, column=1)
        self.txtTotal.bind("<KeyRelease>", self.verificar_contenido5)

        lblResultado = ttk.Label(self.ejercicio5, text="Resultado:")
        lblResultado.grid(row=6, column=1)
        self.txtResultado5 = tk.Text(self.ejercicio5, wrap=tk.WORD, width=40, height=5)
        self.txtResultado5.grid(row=7, column=1)
        self.txtResultado5.config(state="normal")
        self.btnEjercicio5 = ttk.Button(self.ejercicio5, text="Ejecutar Ejercicio", command=self.GenerarLenguaje)
        self.btnEjercicio5.grid(row=8, column=1)
        self.btnEjercicio5.config(state="disabled")
    ...
    def verificar_contenido5(self, event):
        if self.txtLenguaje.get().strip() == "" or self.txtTotal.get().strip() == "":
            self.btnEjercicio5.config(state="disabled")
        else:
            self.btnEjercicio5.config(state="normal")
    ...
    def GenerarLenguaje(self):
        lenguaje = self.txtLenguaje.get().split(',')
        total    = int(self.txtTotal.get())
        resultado5A = self.alpha.GenerarLenguaje(lenguaje,total)
        self.txtResultado5.delete(1.0, tk.END)
        self.txtResultado5.insert(tk.END, "Lenguaje Generado \n")  # Agrega una línea en blanco
        for item in resultado5A:
            self.txtResultado5.insert(tk.END, item + "\n")
